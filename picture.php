	...
	<div>
		<div class='row'>
			...
			<div style = 'height:300px; line-height:30px; float:right; margin-top:100px; margin-right:200px;'><br>
				...
				<div style="float:left;">作者的話：</div><br><div style='font-size:14px'><?php echo nl2br($wwrod); ?></div></div>
			...
			<div style='border-radius:50px;  float:left; border-color:#CED1DB; border-style:solid; border-width:3px; margin-left: 300px; text-align:center; line-height:200%; '>
				<a href="picture.php?url=<?php if ($ptr==1) echo substr($_SESSION["file$tail"],-30,30); else echo substr($_SESSION["file$up"],-30,30) ;?>&<?php if (isset($_GET['user'])) echo "user=".$user; elseif (isset($_SESSION["account"])) echo "account=".$_SESSION["account"];?>" style="display:block; color:white;">上一張
				</a>
			</div>
			...
			<div style='border-radius:50px; float:left; border-color:#CED1DB; border-style:solid; border-width:3px; margin-left: 460px; text-align: center; line-height:200%'>
				<a href="picture.php?url=<?php if ($ptr==$tail) echo substr($_SESSION["file1"],-30,30); else echo substr($_SESSION["file$down"],-30,30) ;?>&<?php if (isset($_GET['user'])) echo "user=".$user; elseif (isset($_SESSION["account"])) echo "account=".$_SESSION["account"];?>" style="display:block; color:white; ">下一張
				</a>
			</div>
